        ...
        self.requestManager.getDirection(start: start, end: end) { [weak self] (result) in
            ...
            case let .success(points):
                guard let points = points, let startLocation = self.startLocation,
                    let endLocation = self.endLocation, let driver = self.drivers.first else { return }
...
extension MapViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: Constants.driverCell, for: indexPath) as! DriverCollectionViewCell
...
extension MapViewController: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        return CGSize(width: Int(width) - 50, height: Constants.heightCell)
    }
